...
export class AddProjectAndDeveloperIdToProperties1693248200000
  implements MigrationInterface
{
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    if (table) {
      ...
      const developerForeignKey = table.foreignKeys.find(
        (fk) => fk.columnNames.indexOf('developer_id') !== -1,
      );
      ...
      // Remove indexes
      const projectIndex = table.indices.find(
        (idx) => idx.name === 'IDX_PROPERTIES_PROJECT_ID',
      );
      if (projectIndex) {
        await queryRunner.dropIndex('properties', projectIndex);
      }

      const developerIndex = table.indices.find(
        (idx) => idx.name === 'IDX_PROPERTIES_DEVELOPER_ID',
      );
      if (developerIndex) {
