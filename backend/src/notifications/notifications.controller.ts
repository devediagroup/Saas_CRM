...
export class NotificationsController {
  ...
  async markAllAsRead(@User('id') userId: string): Promise<{ message: string }> {
  ...
  @ApiResponse({ status: 201, description: 'Lead assigned notification created', type: Notification })
  ...
  @ApiResponse({ status: 201, description: 'Deal stage notification created', type: Notification })
  async createDealStageChangedNotification(
    @Body() body: { dealId: string; dealName: string; newStage: string; agentId: string },
    @User('companyId') companyId: string,
  ): Promise<Notification> {
  ...
  @ApiResponse({ status: 201, description: 'Activity reminder notification created', type: Notification })
  ...
  @ApiResponse({ status: 201, description: 'Overdue activity notification created', type: Notification })
  ...
  @ApiResponse({ status: 201, description: 'Property inquiry notification created', type: Notification })
  async createPropertyInquiryNotification(
    @Body() body: { propertyId: string; propertyTitle: string; agentId: string },
    @User('companyId') companyId: string,
  ): Promise<Notification> {
  ...
  @ApiResponse({ status: 201, description: 'System notification created', type: Notification })
  async createSystemNotification(
    @Body() body: { title: string; message: string; priority?: NotificationPriority },
    @User('id') userId: string,
    @User('companyId') companyId: string,
  ): Promise<Notification> {
